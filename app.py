...
def rental_days(from_date, to_date):
    """Number of days a rental covers (inclusive)."""
...
def list_equipment():
    # Only return equipment that is NOT under maintenance
    available_equipment = [item for item in EQUIPMENT if item.get("status") != "maintenance"]
